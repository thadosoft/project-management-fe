...
export const login = async (username: string, password: string): Promise<{ accessToken: string; id: string; role: string } | null> => {
  return await fetchData<
    { accessToken: string; id: string; role: string },
    { username: string; password: string }
  >("auth/authenticate", "POST", undefined, { username, password });
};

export const register = async (name: string, email: string, username: string, phoneNumber: string, password: string): Promise<{ accessToken: string; id: string; role: string } | null> => {
  return await fetchData<
    { accessToken: string; id: string; role: string },

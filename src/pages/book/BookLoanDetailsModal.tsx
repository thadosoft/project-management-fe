  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold">{book.bookTitle}</DialogTitle>
        </DialogHeader>

        <div className="space-y-6 mt-4">
          <div className={`px-4 py-3 rounded-md ${statusBadge.bg}`}>
            <p className={`font-semibold text-sm ${statusBadge.text}`}>{statusBadge.label}</p>
          </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="space-y-3">
                    <InfoField label="Tác giả" value={book.approverName} />
                    <InfoField label="Chủ sở hữu" value={book.bookOwner} />
                    <InfoField label="Vị trí" value={book.remarks} />
                  </div>
                </div>
              </div>
            </CardContent>
